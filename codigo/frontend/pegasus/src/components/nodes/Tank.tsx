...
// função que renderiza o nó de Tank
export default function Tank(props: NodeProps) {
  return (
    <Popover.Root>
      <Popover.Trigger asChild>
        <div className="rounded w-[80px] h-[80px] flex items-center justify-center text-center text-white">
          <img src={tank} alt="Tank node" />
          <Handle
            id="bottom"
            type="source"
            position={Position.Bottom}
            className="-bottom-10 left-[47px] w-3 h-3 border-2 bg-blue-400/80"
          />
        </div>
      </Popover.Trigger>
      <Popover.Portal>
