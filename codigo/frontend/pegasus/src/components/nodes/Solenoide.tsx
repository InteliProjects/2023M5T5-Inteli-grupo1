...
import solenoide from "../../assets/img/solenoide.svg";

// função que renderiza o nó de Solenoide
export default function Solenoide(props: NodeProps) {
  return (
    <Popover.Root>
      <Popover.Trigger asChild>
        
        <div className="rounded w-[80px] h-[80px]  flex items-center justify-center text-center text-white">
          <img src={solenoide} alt="Solenoide Node" />
          <Handle
            id="right"
            type="source"
            position={Position.Right}
            className="-right-0 top-[66px] w-3 h-3 border-2 bg-blue-400/80"
          />
          <Handle
            id="left"
            type="source"
            position={Position.Left}
            className="-left-0 top-[66px] w-3 h-3 border-2 bg-blue-400/80"
          />
        </div>
      </Popover.Trigger>
      <Popover.Portal>
